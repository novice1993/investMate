...
import { fetchMKSitemap } from "@/core/infrastructure/news.mk.infra";
import { parseMKSitemap } from "@/core/services/news.mk.service";
...
export async function GET() {
  try {
    ...
    console.log("[/api/news/mk] Transformed News Entities:", JSON.stringify(news, null, 2));

    // 4. 변환된 결과를 클라이언트에 반환
    return NextResponse.json({ news });
  } catch (error) {
    console.error(`[api/news/mk] Error:`, error);
